...
export const parseScheduleInput = (
    payload: unknown
): { input: ScheduleInput; errors: string[] } => {
...
export const computeCapacityWarning = (input: ScheduleInput): string | null => {
    const minDaysOff = input.minDaysOff ?? 0;
    const totalRequired =
        input.periodDays * input.shifts.length * input.buildings.length * input.guardsPerShift;
